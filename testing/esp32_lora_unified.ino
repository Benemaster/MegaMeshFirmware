...
// Advanced mesh state
bool meshEncryptionEnabled = true;
...
void handleCommand(String raw, bool fromBT = false)
{
    ...
    if (cmd == "help")
    {
        sendMsg("{\"cmds\":\"setup|info|set|device|save|init|reboot|bt off|mesh scan|mesh peers|mesh key <hex>|mesh enc <on|off>|weather on|weather off|weather interval <ms>|weather add <pin> <a|d>|weather clear|weather now|send <msg>|sendto <nodeId> <msg>\"}");
        return;
    }
    if (cmd == "show")
    {
    ...
    if (cmd.startsWith("device "))
    {
        ...
        deviceType.trim();
        if (deviceType == "heltec")
            setDefaultsHeltec();
        else if (deviceType == "wroom")
            setDefaultsWroom();
        else
        {
    ...
    if (cmd == "save")
    {
        if (!setupModeActive)
        {
            sendMsg("{\"evt\":\"setup_required\",\"hint\":\"send setup first\"}");
            return;
        }
    ...
    if (cmd == "init")
    {
        ...
        // Module(cs/nss, irq, rst, busy)
        Module *mod = new Module(cfg.nssPin, cfg.dio1Pin, cfg.rstPin, cfg.dio0Pin);
        radioPtr = new SX1262(mod);
        int16_t state = radioPtr->begin(cfg.frequency, cfg.bandwidth, cfg.spreadingFactor, cfg.codingRate, cfg.syncWord, cfg.pwr, cfg.preambleLength, cfg.tcxoVoltage, false);
...
void sendSetupInfo()
{
    ...
    j += ",\"device\":\"" + String(cfg.deviceType == 0 ? "heltec" : "wroom") + "\"";
    j += ",\"fields\":[";
    j += "{\"k\":\"freq\",\"v\":" + String(cfg.frequency, 1) + ",\"unit\":\"MHz\",\"min\":137.0,\"max\":1020.0}";
    j += ",{\"k\":\"bw\",\"v\":" + String(cfg.bandwidth, 1) + ",\"unit\":\"kHz\",\"opts\":\"7.8|10.4|15.6|20.8|31.25|41.7|62.5|125|250|500\"}";
    j += ",{\"k\":\"sf\",\"v\":" + String(cfg.spreadingFactor) + ",\"min\":6,\"max\":12}";
    j += ",{\"k\":\"cr\",\"v\":" + String(cfg.codingRate) + ",\"opts\":\"5|6|7|8\"}";
    j += ",{\"k\":\"pwr\",\"v\":" + String(cfg.pwr) + ",\"unit\":\"dBm\",\"min\":2,\"max\":22}";
    ...
    j += ",{\"k\":\"weather_interval\",\"v\":" + String(weatherIntervalMs) + ",\"unit\":\"ms\",\"min\":500,\"max\":600000}";
    j += ",{\"k\":\"weather_sensors\",\"v\":" + String(sensorCount) + ",\"type\":\"count\"}";
    j += "],\"cmds\":\"setup|info|set|device|save|init|reboot|bt off|mesh scan|mesh peers|mesh key|mesh enc|weather on|weather off|weather interval|weather add|weather clear|weather now|send|sendto\"";
    j += "}";
    sendMsg(j);
}
...
// Incoming packet handler: decode advanced mesh frames and keep raw compatibility events.
